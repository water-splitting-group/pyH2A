...
from .Utilities.input_modification import convert_input_to_dictionary, process_input, process_table, insert, read_textfile, set_by_path, execute_plugin
from .Utilities.find_nearest import find_nearest as fn
...
def MACRS_depreciation(plant_years, depreciation_length, annual_depreciable_capital):
	...
	idx_macrs = fn(macrs[0][1:], depreciation_length)[0] 
...
class Discounted_Cash_Flow:
	...
	**Plugins**
	...
	def inflation(self):
		...
		plant_idx = fn(plant_cost, [self.fin['current year capital costs']['Value'], 
												 self.fin['basis year']['Value']])
		gdp_idx = fn(gdp_deflator_price, [self.fin['ref year']['Value'], 
													   self.fin['current year capital costs']['Value']])
		labor_idx = fn(labor_price, [self.fin['ref year']['Value'], 
												  self.fin['basis year']['Value']])
		chemical_idx = fn(chemical_price, [self.fin['ref year']['Value'], 
														self.fin['basis year']['Value']])
	...
	def replacement_costs(self):
		...
		self.start_idx = fn(self.plant_years, 0)[0]
