...
.. code-block:: Python
	...
	class Example_Plugin:
		...
		Parameters
		----------
		Example Table Name > Example table row > Value : float
			Example input read from input file by plugin.

		Returns
		-------
		Example Output Table > Example output row > Value : float
		...
		def method(self):
			...
			self.attribute = self.input_dict_resolved['Example Table Name']['Example table row']['Value']
...
It then runs ``self.method()``, in which ``self.attribute`` is equal to the Quantity found in ``Example Table Name > Example table row > Value``.
