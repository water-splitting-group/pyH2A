...
class Foreground_LCI_Database_Plugin_V2:
    ''' Processing of foreground LCI data for the LCA calculation
    ...
    def __init__(self, dcf, print_info):
        process_table(dcf.inp, 'LCA PV+E Database', 'Value')

        self.activities()
        self.exchanges(dcf)
        self.LCI_database(dcf, self.activities(), self.exchanges())

        insert(dcf, 'Table', 'Row', 'Value', 
               self.atribute, __name__, print_info = print_info)
    ...
    def exchanges(self, dcf):
        exchanges = {
            "exchanges": [
                ...
                {
                    "input": "sea water",
                    "amount": dcf.inp['LCA PV+E Database']['Amount of sea water (m3)']['Value'],  # !!!! specify the correct amount
                    "type": "biosphere",
                    "unit": "cubic meter",
                    "activity": "production_of_hydrogen"
                }, 
