from pyH2A.Utilities.input_modification import hourly_to_yearly_power_quantity
...
class Power_Management_Explicit_Battery_Plugin:
    ...
    def __init__(self, dcf, print_info):
        ...
        self.input_dict_resolved = input_resolver_function(input_dict, dcf, 'Power_Management_Explicit_Battery_Plugin')  

        if 'Power Consumption' in self.input_dict_resolved:    
            self.calculate_consumers()
            self.calculate_electricity_cost()

        output_inserter_function(output_dict, self, dcf, 'Power_Management_Explicit_Battery_Plugin') 
    ...
    def calculate_consumers(self):
        ...
        try:
            available_energy_yearly = hourly_to_yearly_power_quantity(self.input_dict_resolved['Power Generation']['Available energy (hourly)']['Value'])
        except KeyError:
        ...
        try:
            main_unfulfilled_yearly = hourly_to_yearly_power_quantity(self.input_dict_resolved['Hourly Consumer Profile']['Unsatisfied demand']['Value'])
        except KeyError:
