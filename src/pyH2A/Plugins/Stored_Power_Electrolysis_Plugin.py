...
input_dict = {
    ...
    "Electrolyzer": {
        ...
        "H2 production (yearly)": { # this will be removed in PR117, replaced by Design output by year hereafter
            "Value": {
                "type": {np.ndarray},
                "bounds": (0, None)
            },
...
class Stored_Power_Electrolysis_Plugin:
    ...
    Electrolyzer > H2 production (yearly) > Value : nd.array # PR117: will be replaced by Design output by year herafter
        Yearly hydrogen production.
    Technical Operating Parameters and Specifications > Design output by year > Value : nd.array
        Yearly hydrogen production, excluding the effect of operating capacity factor.
    ...
    -------
    Technical Operating Parameters and Specifications > Design output by year > Value : nd.array
        Design output by year in mass of H2, calculated from installed 
