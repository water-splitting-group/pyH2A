...
class Solar_Thermal_Plugin:
    ...
    def __init__(self, dcf, print_info):
        ...
        insert(dcf, 'Raw product gas', 'Flowrate', 'Value',
                self.Mass_flowrate, __name__, print_info = print_info)                 
        ...
        insert(dcf, 'Raw product gas', 'Composition', 'Value',
                self.Mass_fractions, __name__, print_info = print_info)                  
        insert(dcf, 'Raw product gas', 'Enthalpy', 'Value',
                self.Enthalpy, __name__, print_info = print_info)  
    ...
    def calculate_product_fluid_properties(self, dcf):    
        '''Hardcoded values for the moment. Ultimately they would be the result of a physics-based calculation.
        '''
        
        self.Pressure = 1.01315 
        self.Temperature = 333 # 60 degC
        self.Mass_fractions = {'H2':0.3, 'H2O':0.7} 
        self.Species = {'H2', 'H2O'}
        self.Hydrogen_mol_fraction = (self.Mass_fractions['H2']/FP.MW['H2'])/(self.Mass_fractions['H2']/FP.MW['H2']+self.Mass_fractions['H2O']/FP.MW['H2O'])
        self.Enthalpy = FP.Enthalpy(self.Temperature, self.Pressure, self.Mass_fractions, 'V')
        self.Mass_flowrate = dcf.inp['Technical Operating Parameters and Specifications']['Design Output per Day']['Value'] / (86400*self.Mass_fractions['H2'])