...
Solver: JAX (jax.scipy.linalg.solve)
"""
...
def solve(matrix, f):
    """
    Solve matrix * x = f using JAX (jax.scipy.linalg.solve).

    Converts sparse matrices to dense before solving. Falls back to
    numpy.linalg.solve if jax is not installed.
    """
    if jax is None:
    ...
    A_j = jnp.array(dense, dtype=jnp.float64)
    b_j = jnp.array(rhs, dtype=jnp.float64)
