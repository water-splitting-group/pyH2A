...
class pyH2A:
	...
	def __init__(self, input_file, output_directory, print_info = False):
		...
		fu.set_Functional_Unit(self.inp['Functional Unit']['Functional unit'])
		self.base_case = Discounted_Cash_Flow(self.input_file, print_info = print_info)

		self.meta_modules = {}
		self.meta_workflow(self.meta_modules)

		print(f'Levelized cost of final product (base case): {self.base_case.final_product_cost} $ /',fu.Functional_Unit, fu.Ref)
