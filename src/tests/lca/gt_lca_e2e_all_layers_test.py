...
def _run_pyH2A_cold(input_file_stem):
    """Run the full pyH2A pipeline against a scenario input file, first
    clearing both the RAM cache and the target folder's on-disk cache."""
...
def test_2layer_result_matches_reference(input_file_stem, impact_name, expected_value, expected_unit):
