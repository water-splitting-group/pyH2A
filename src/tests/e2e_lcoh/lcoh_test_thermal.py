import pytest
from pyH2A.run_pyH2A import pyH2A


@pytest.mark.parametrize(
    "case",
    [
        {
            "input": {
                "input_file": "src/tests/end_to_end/Thermal_Base.md",
                "output_directory": "src/tests/end_to_end/",
            },
            "expected": {"lcoh": 3.270581409704611},
        },
        {
            "input": {
                "input_file": "src/tests/end_to_end/Thermal_bases/thermal_capital_cost_test.md",
                "output_directory": "src/tests/end_to_end/",
            },
            "expected": {"lcoh": 3.7271856030880977},
        },
        {
            "input": {
                "input_file": "src/tests/end_to_end/Thermal_bases/thermal_labor_operating_cost_test.md",
                "output_directory": "src/tests/end_to_end/",
            },
            "expected": {"lcoh": 10.202394944067255},
        },
        {
            "input": {
                "input_file": "src/tests/end_to_end/Thermal_bases/thermal_production_test.md",
                "output_directory": "src/tests/end_to_end/",
            },
            "expected": {"lcoh": 3.6789953252124796},
        },
        {
            "input": {
                "input_file": "src/tests/end_to_end/Thermal_bases/thermal_replacement_test.md",
                "output_directory": "src/tests/end_to_end/",
            },
            "expected": {"lcoh": 3.2544152106369726},
        },
        {
            "input": {
                "input_file": "src/tests/end_to_end/Thermal_bases/thermal_solar_thermal_test.md",
                "output_directory": "src/tests/end_to_end/",
            },
            "expected": {"lcoh": 1.6440086736860908},
        },
        {
            "input": {
                "input_file": "src/tests/end_to_end/Thermal_bases/thermal_variable_operating_cost_test.md",
                "output_directory": "src/tests/end_to_end/",
            },
            "expected": {"lcoh": 8.671474184241813},
        },
    ],
    ids=[
        "PV_E_Base",
        "Thermal_Capital_Cost",
        "Thermal_Labor_Operating_Cost",
        "Thermal_Production",
        "Thermal_Replacement",
        "Thermal_Solar_Thermal",
        "Thermal_Variable_Operating_Cost",
    ],
)
def test_e2e_lcoh(case):
    """
    End-to-end regression test for Levelized Cost of Hydrogen (LCOH).

    This test runs the full pyH2A workflow using reference input files
    and asserts that the computed LCOH matches the master-branch
    reference value within tight numerical tolerance.

    Purpose:
    - Detect unintended economic logic changes
    - Detect numerical drift
    - Protect financial model stability
    """

    input_data = case["input"]

    result = pyH2A(input_data["input_file"], input_data["output_directory"])

    # Very strict tolerance to detect economic regression
    tolerance = 1e-12

    assert result.base_case.h2_cost == pytest.approx(
        case["expected"]["lcoh"], rel=tolerance
    )
